    ...
    >
      ...
      <div style={{ marginBottom: '5px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'baseline', gap: '4px', marginTop: '3px' }}>
          <p style={{ fontSize: '8pt', color: '#333', margin: 0, flexShrink: 0 }}>Paciente:</p>
          <div style={{ flex: 1, borderBottom: '1px solid #aaa' }}>
            <p style={{ fontSize: '9pt', fontWeight: 600, color: '#111', margin: 0, paddingLeft: '2px' }}>
        ...
        >
          {meds.map((med, i) => (
            <div key={i} style={{ marginBottom: i < meds.length - 1 ? '6px' : 0 }}>
              <p style={{ fontSize: '9.5pt', fontWeight: 700, color: '#1B5E35', margin: 0, lineHeight: 1.3 }}>
                {i + 1}. {med.name}
              </p>
              {med.presentation && (
                <p style={{ fontSize: '8pt', color: '#444', margin: 0, paddingLeft: '10px', lineHeight: 1.3 }}>
                  {med.presentation}
                </p>
              )}
              {med.posology && (
                <p style={{ fontSize: '8pt', color: '#333', margin: 0, paddingLeft: '10px', lineHeight: 1.3 }}>
                  {med.posology}
                </p>
              )}
              {med.quantity && (
                <p style={{ fontSize: '8pt', color: '#666', margin: 0, paddingLeft: '10px', lineHeight: 1.3 }}>
      ...
      <div style={{ marginTop: 'auto', paddingTop: '6px', borderTop: '1px solid #eee' }}>
        ...
        <p style={{ fontSize: '8pt', fontWeight: 700, color: '#1B5E35', margin: 0 }}>
          {profile?.full_name || ''}
        </p>
        <p style={{ fontSize: '7.5pt', color: '#666', margin: 0 }}>
          {profile?.specialty || ''}
        </p>
      </div>
    </div>
